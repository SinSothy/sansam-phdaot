    ...
    >
      <div className="sticky top-0 z-30 bg-white/95 backdrop-blur-sm border-b border-surface-container mb-6 -mx-8 px-8 py-2 flex items-center gap-1 shadow-sm">
        ...
        {linkDialogOpen && (
          <div className="absolute top-14 left-64 bg-white rounded-xl shadow-lg border border-surface-container p-4 w-80 flex flex-col gap-3 z-50">
            <h3 className="text-sm font-semibold text-on-surface">{t('insertLink')}</h3>
            <div className="flex flex-col gap-1">
              <label className="text-xs text-secondary font-medium">{t('displayLinkText')}</label>
              <input 
                type="text" 
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-secondary font-medium">{t('displayLinkAddress')}</label>
              <input 
                type="url" 
                className="w-full text-sm border border-outline-variant rounded p-2 focus:ring-1 focus:ring-primary outline-none" 
                value={linkData.url}
            ...
            <div className="flex justify-end gap-2 mt-1">
              <button onClick={() => setLinkDialogOpen(false)} className="px-3 py-1.5 text-xs font-medium text-secondary hover:bg-surface-container rounded transition-colors">{commonT('cancel')}</button>
              <button onClick={submitLink} className="px-3 py-1.5 text-xs font-medium bg-primary text-white rounded hover:bg-primary/90 transition-colors shadow-sm">{commonT('confirm')}</button>
            </div>
          </div>
        )}
      </div>
      ...
      <header className="mb-10 shrink-0 bg-white/80 backdrop-blur p-6 rounded-2xl border border-surface-container shadow-sm w-fit max-w-4xl relative z-20">
        <div className="flex items-center gap-2 text-xs text-secondary mb-4 uppercase tracking-tighter font-semibold">
          <span>{t('titleFor', {date: dateKey})}</span>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-xs text-secondary font-medium">{t('lastEdited', {time: 'today'})}</span>
        </div>
      </header>
