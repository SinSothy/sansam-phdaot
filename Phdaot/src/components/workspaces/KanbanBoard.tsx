...
// Setup Mock Data
const INITIAL_BOARD_DATA: BoardColumnData[] = [
  {
    id: 'col-1',
    title: 'Backlog',
...
export function KanbanBoard() {
  const [boardData, setBoardData] = useState<BoardColumnData[]>(INITIAL_BOARD_DATA);
  ...
  const addList = () => {
    ...
    setBoardData([
      ...boardData,
      { id: listId, title: 'New List', highlightBadge: false, cards: [] }
    ]);
  };
  ...
  const addCard = (listId: string) => {
    ...
    setBoardData(boardData.map(col => {
      if (col.id === listId) {
        return {
          ...col,
          cards: [...col.cards, { id: cardId, title: 'New task' }]
        };
      }
      return col;
    }));
                ...
                <div className="flex items-start gap-6 min-w-max h-full pb-4">
                  {boardData.map((col, index) => (
                    <KanbanColumn 
                      ...
                      cards={col.cards as KanbanCardProps[]}
